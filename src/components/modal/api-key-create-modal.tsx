...
// Fixed expiration presets (A-option). Null = never expires. Keys are i18n
// keys under `modal.apiKeyCreate.*` so the button labels localise.
...
export default function ApiKeyCreateModal() {
  ...
  // Modal can only be opened from pages under /app/:teamId, so teamId is
  // guaranteed present at render time. The null-check below is a defensive
  // guard in case the modal somehow leaks outside team scope.
