...
//
// Adapted from ste/frontend_web. TMS uses `useCurrentUser()` returning a
// `UserEntity` (rather than ste's `useSession()` returning `{ user }`).
// Field mapping:
//   ste session.user.name              -> tms user.name
//   ste session.user.email             -> tms user.email
//   ste session.user.profile_image_url -> not in TMS UserEntity (initials only)
//
// Sign-out follows TMS sidebar pattern: `clearAuth()` + redirect to /sign-in.
// Name / password change render as stubs that toast "coming soon" — no
// backend mutation wired yet.
...
export default function ProfileModal() {
  ...
  // Defensive: if the user clears while the modal is open (sign-out from
  // anywhere, session expiry), make sure the dialog state closes too.
  useEffect(() => {
    if (!user && profileModal.isOpen) {
    ...
    <>
      <Dialog open={profileModal.isOpen} onOpenChange={handleOpenChange}>
        ...
        >
          ...
          >
            {user && <ProfileBody user={user} onOpen={setSubDialog} />}
          </div>
        </DialogContent>
      </Dialog>
    ...
    <div className="flex flex-col gap-0">
      <div className="flex items-center gap-3 px-6 py-5">
        ...
        <div className="flex min-w-0 flex-col">
          ...
          <p className="truncate text-xs text-black/55">{email}</p>
        </div>
        <button
          type="button"
          onClick={handleSignOutClick}
          className="ml-auto flex h-9 shrink-0 items-center gap-1.5 rounded-lg border border-black/10 bg-white px-3 text-xs font-medium text-black/60 transition-colors hover:border-black/20 hover:bg-black/[0.02] hover:text-black"
        >
...
function ChangeNameDialog({
  ...
  // TODO: wire to a real `useUpdateProfile` mutation when the backend
  // exposes one. Currently a visual stub — the toast indicates the feature
  // is pending so the modal stays useful.
  const handleSave = () => {
...
function ChangePasswordDialog({
  ...
  // TODO: wire to a real `useChangePassword` mutation. Visual stub for now.
  const handleSave = () => {
...
function DeleteAccountDialog({
  ...
  // TODO: wire to a real `useDeleteAccount` mutation. Currently a stub —
  // the request is "received" and toasted.
