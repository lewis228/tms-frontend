...
export default function RateSettingEditorModal() {
  ...
  const [rateType, setRateType] = useState<RateType>("FLAT_RATE");
  ...
  useEffect(() => {
    ...
    if (modal.type === "CREATE") {
      ...
      const r = modal.rateSetting;
      setName(r.name);
      setRateType(r.rateType);
      setFlatAmount(r.flatAmount ?? "");
      setRatePercent(r.ratePercent ?? "");
      setRatePerMile(r.ratePerMile ?? "");
  ...
  const handleSave = () => {
    ...
    const perMile = rateType === "PER_MILE" ? ratePerMile.trim() || null : null;
