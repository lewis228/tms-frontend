...
export default function SettingsTagsPage() {
  ...
  const { data: tags = [], error, isPending } = useTeamTagsData(teamId);
  ...
  const { mutate: createTagMutation, isPending: isCreatingPending } =
    useCreateTag({
      onSuccess: () => {
        setNewTagName("");
        setIsCreating(false);
  ...
  const filteredTags = useMemo(() => {
    ...
    const sorted = [...base].sort((a, b) => {
      ...
      // recent-desc: newest updated_at first; falls back to created_at.
  ...
  const handleCreateTag = () => {
    ...
    createTagMutation({ name: trimmedNew });
  };

  const handleRenameTag = (id: number, nextName: string) => {
    const trimmed = nextName.trim();
    if (trimmed === "") return;
    updateTagMutation({ tagId: String(id), name: trimmed });
  };

  const handleDeleteTag = (id: number) => {
    deleteTagMutation(String(id));
  };

  // Zero-tag state when the search input is empty → show the hero "no data"
  // layout instead of the table shell. With a search term in play we still
  // render the toolbar + table shell so the user can clear the filter.
  const showHeroEmpty =
    !isPending && !error && tags.length === 0 && query.trim() === "";
    ...
    <div className="flex flex-col gap-7 p-7">
      ...
      {error ? (
        ...
        <section className="flex flex-col gap-4 rounded-2xl border border-black/10 bg-white p-5">
          ...
          {/* Trailing row: either the create-tag input (when active) or a
              muted "+ Add tag" trigger. Kept inside the card so it reads as
              the table's last row rather than a separate control. */}
          {isCreating ? (
            <CreateTagInlineRow
              value={newTagName}
              ...
              isSubmitting={isCreatingPending}
            />
          ) : (
            <button
              type="button"
...
// ---------------------------------------------------------------------------
// Hero empty state — shown when the team has zero tags and no search query
// ---------------------------------------------------------------------------
...
function SortSelect({
  ...
  const { t } = useTranslation();
  // `used-desc` is no longer surfaced because the backend doesn't track
  // per-tag usage counts yet. Ordered pair of (name-asc / name-desc /
  // recent-desc) covers the common ask.
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-dashed border-black/15 bg-black/[0.015] px-4 py-3">
      ...
      <Button
        type="button"
        size="sm"
        disabled={value.trim() === "" || isDuplicate || isSubmitting}
        onClick={onSubmit}
...
function TagTableRow({
  ...
  return isEditing ? (
    <EditableTagRow
      // Keyed on tag.name so the draft state resets if the tag is renamed
      // out from under us — avoids a setState-in-useEffect sync pattern.
      key={tag.name}
      tag={tag}
      onCancelEdit={onCancelEdit}
  ...
  return (
    <>
      <span className="text-sm text-black/50">
        {tag.created_at ? formatDate(tag.created_at) : t("common.none")}
      </span>
      <span className="text-sm text-black/50">
        {tag.updated_at ? formatDate(tag.updated_at) : t("common.none")}
      </span>
    </>
  );
}
