...
// Team-scoped display preferences. For the MVP these live in localStorage so
// the page can stabilise the UX before a backend column exists. Timezone is
// also mirrored onto the TeamEntity (backed by `PATCH /team`) so other team
// members see the same zone; the local copy here is the cached fallback
// used by format.ts when no team is loaded yet.
//
...
type State = {
  timezone: string; // IANA id. Defaults to browser detection on first load.
