...
// slot by a **domainKey** string (e.g. `"deliveryOrder.list"`). The store
// remembers per-key which columns are visible + the display order.
//
// Fixed columns (e.g. row checkbox, the Actions cell) are **not**
...
//
// Usage example (pseudo-code — wiring is opt-in per grid):
//
//   import {
...
//
//   const COLUMNS = [
//     { key: "blNumber",  labelKey: "do.col.blNumber",  defaultVisible: true },
//     { key: "customer",  labelKey: "do.col.customer",  defaultVisible: true },
//     { key: "container", labelKey: "do.col.container", defaultVisible: false },
//   ];
//
//   const ensure  = useEnsureGridColumnConfig();
//   const setVis  = useSetGridColumnVisibility();
//   const config  = useGridColumnConfig("deliveryOrder.list");
//
//   useEffect(() => ensure("deliveryOrder.list", COLUMNS), [ensure]);
//
//   // Render only visible columns:
//   COLUMNS.filter((c) => config?.visible[c.key] ?? c.defaultVisible)
//          .map((c) => <th key={c.key}>{t(c.labelKey)}</th>);
